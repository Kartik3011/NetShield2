...
import random # <-- FIX 1: Import random for delay
...
for i, j in zip(video_Link, video_titles):
    with details_container:
        ...
        with st.spinner(f"Searching for news context using query: '{query}'"):
            try:
                # FIX 2: Add randomized delay to prevent rate limiting/blocking
        ...
        with st.spinner("Summarizing news content..."):
            try:
                # FIX 3: Concatenate the content of all News objects into a single string
                full_news_content = " ".join([article.content for article in ns])

                # Pass the concatenated string to the summarizer
                newssum=sumz.sumup(full_news_content)
                
            except NameError:
                newssum = "NO_NEWS_SUMMARY" # Fallback if module fails
            except Exception as e:
                st.error(f"Error during News Summarization: {e}")
                newssum = "NO_NEWS_SUMMARY"
        ...
        with st.spinner("Running Llama 3 for Final Validation.  This might take a while for longer videos..."):
            try:
                # Use the summarized content for validation
